...
public class JavaIncrementalAnalyzeManager extends AsyncIncrementalAnalyzeManager<State, Long> {
    ...
    public List<CodeBlock> computeBlocks(
            Content text,
            AsyncIncrementalAnalyzeManager<State, Long>.CodeBlockAnalyzeDelegate delegate) {
                    ...
                    if (type > 0) {
                        if (isStart(token)) {
                            bracketsStack.push(
                                    IntPair.pack(
                                            type,
                                            text.getCharIndex(i, IntPair.getSecond(tokenRecord))));
                        } else {
                                    ...
                                    while (!bracketsStack.isEmpty()) {
                                        ...
                                        if (IntPair.getFirst(record) == 3) {
                                            brackets.add(
                                                    IntPair.getSecond(record),
                                                    text.getCharIndex(
                                                            i, IntPair.getSecond(tokenRecord)));
                                            break;
                                        }
                                    }
                                }
    ...
    private static boolean isStart(int token) {
        return token == Tokens.LBRACE.ordinal()
                || token == Tokens.LBRACK.ordinal()
                || token == Tokens.LPAREN.ordinal();
    }
    ...
    @Override
    public LineTokenizeResult<State, Long> tokenizeLine(
                ...
                case VAR:
                    ...
                    spans.add(
                            Span.obtain(
                                    offset,
                                    TextStyle.makeStyle(
                                            EditorColorScheme.KEYWORD, 0, false, false, false)));
                    break;
                case ABSTRACT:
                case ASSERT:
                case CLASS:
