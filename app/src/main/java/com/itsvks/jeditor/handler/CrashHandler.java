package com.itsvks.jeditor.handler;
...
import com.itsvks.jeditor.activities.CrashActivity;
...
public class CrashHandler implements UncaughtExceptionHandler {
        ...
        if (Looper.myLooper() != null) {
            ...
            while (true) {
                try {
                    handler.post(
                            () -> {
                                mContext.startActivity(new Intent(mContext, CrashActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
                                Toast.makeText(mContext, "App crashed!", Toast.LENGTH_SHORT).show();
                            });
                    Looper.loop();
                } catch (Throwable t) {
    ...
    private void saveCrashInfo(String threadName, Throwable ex) {
        ...
        sb.append("Crash at ")
                .append(timestamp)
                .append("(timestamp) in thread named '")
                .append(threadName)
                .append("'\n");
        sb.append("Local date and time:")
                .append(SimpleDateFormat.getDateTimeInstance().format(new Date(timestamp)))
                .append('\n');
        ...
        try {
            ...
            FileOutputStream fos = mContext.openFileOutput("crash-info.log", Context.MODE_APPEND);
